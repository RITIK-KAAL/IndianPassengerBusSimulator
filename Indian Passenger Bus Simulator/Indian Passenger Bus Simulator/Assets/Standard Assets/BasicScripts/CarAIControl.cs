...
/*        [SerializeField]
        ...
        public void FixedUpdate()
        {
            ...
            else
            {
                ...
                // AI Car threshold Brake if any car is in the fornt of AI to avoid Car Clash.
                RaycastHit hit;
                if (Physics.Raycast(Dashboard.position, transform.forward, out hit, Distance))
                {
/*                    Debug.DrawRay(Dashboard.position, hit.point, Color.red);*/
                    if (hit.transform.tag == "Player")
                    {
                        var otherAI = hit.rigidbody.GetComponent<CarAIControl>();
                        m_Rigidbody.drag = (40f);
                        if(otherAI != null) 
                        {
                ...
                if (Physics.SphereCast(Dashboard.position, radius, transform.forward, out hit, Distance))
                {
/*                    audioSource.Play();*/
                    if (hit.transform.tag == "Player")
                    {
